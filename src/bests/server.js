...
app.get("/*.md", async (req, res, next) => {
  ...
  res.send(
    await HTML.string`<!doctype html>
<html>
<head>
<title>${req.url}</title>
<meta charset="utf-8" />
...
app.get("/favicon.ico", (req, res) => {
  res.sendFile(__dirname + "/src/icon.png");
});
      ...
      body = await Promise.race([
        (async () => {
          const result = await HTML.string`<div>
            ${BestsRouter.of({
              url: new url.URL(req.path, `http://${req.hostname}/`),
            })}
          </div>`;
  ...
  return res.send(
    index
      ...
      .replace(
        ...
        await recursiveReaddir("src").then(files =>
          files
            .filter(s => /\.js$/i.test(s) && !s.includes("-node"))
            .map(
              s => `import ${JSON.stringify(s.replace(/^src\//, "/assets/"))};`,
            )
            .sort()
            .join("\n"),
        ),
