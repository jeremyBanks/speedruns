...
export const RunDuration: React.FC<{ ms: number }> = ({ ms }) => {
  if (ms) {
    ...
    if (mPart > 0 || pieces.length) {
      pieces.push(`${String(mPart).padStart(2, pieces.length ? "0" : " ")}m`);
    }
    pieces.push(`${String(sPart).padStart(2, pieces.length ? "0" : " ")}`);
    if (msPart) {
