...
export class HTMLPieces {
  constructor(pieces) {
    ...
    for (const piece of this.pieces)
      if (!((typeof piece === 'string') || (piece && typeof piece.then === 'function')))
  ...
  async *flatStringPieces() {
    for (const piece of this.pieces) {
      ...
      if (typeof syncPiece === 'string') {
        ...
        for await (const subPiece of syncPiece.flatStringPieces()) {
          yield syncPiece;
        }
      } else {
        throw new ThisShouldNeverHappenError(
  ...
  fragmentAndDone() {
    ...
    for (const piece of this.pieces) {
      if (typeof piece === 'string') {
        ...
        stringPieces.push(`<${placeholderTagName} class="${placeholderClassName}"></${placeholderTagName}>`);
    ...
    for (const [selector, replacement] of placeholders) {
      ...
      if (matches.length !== 1) {
        throw new SyntaxError(
          "Probably got async element value but not expecting element.\n" +
