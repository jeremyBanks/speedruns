...
export const devAwaitDeep = async (rootValue, baseTimeout = 4096, earlyTimeout = new Promise(() => 0)) => {
  const timeout = Promise.race(earlyTimeout.then(() => ({
    '⏱️': 'Pending Promise',
    message: `still pending after timeout`,
  ...
  const awaitDeepEach = async value => {
    ...
    if (Array.isArray(value)) {
      ...
      return result;
    } else if (value[Symbol.asyncIterator] || value[Symbol.iterator]) {
      const children = [];
      const done = (async() => {
        for await (const child of value) {
          children.push(awaitDeepEach(child));
        }
      })();
      return Promise.race([
