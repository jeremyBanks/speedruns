...
const devAwaitDeep = async (rootValue, maxTimeout = 4096) => {
  const rootTimeout = new Promise(resolve => setTimeout(() => {
    resolve({
      '⚪': 'Pending Promise',
      message: `still pending after ${maxTimeout}ms`,
    });
  }, maxTimeout));

  const awaitDeepEach = async (value, parentTimeout) => new Promise(resolve => {
    const timeout = Promise.resolve(parentTimeout);
    
    // do child stuff, including setting up their callbacks, then...
    
    return Promise.race(
       result,
      timeout.then(async (result) => {
        await allOfTheChildrenWhoShouldBeCancelledToo;
        return result; // or do we return the children now that we've dealt with them?
      });
    )
  });
  
  return awaitDeepEach(value, rootTimeout);
};
